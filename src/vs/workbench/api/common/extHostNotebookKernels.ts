...
export class ExtHostNotebookKernels implements ExtHostNotebookKernelsShape {
	...
	$acceptSelection(handle: number, uri: UriComponents, value: boolean): void {
		...
		if (obj) {
			obj.onDidChangeSelection.fire({
				selected: value,
				notebook: this._extHostNotebook.lookupNotebookDocument(URI.revive(uri))! // {{SQL CARBON EDIT}}
			});
		}
	}
	...
	async $executeCells(handle: number, uri: UriComponents, handles: number[]): Promise<void> {
		...
		for (let cellHandle of handles) {
			const cell = document.cellAt(cellHandle); // {{SQL CARBON EDIT}}
			if (cell) {
				cells.push(cell); // {{SQL CARBON EDIT}}
			}
		}

		try {
			await obj.controller.executeHandler.call(obj.controller, cells, document, obj.controller); // {{SQL CARBON EDIT}}
		} catch (err) {
			//
			console.error(err);
		}
	...
	async $cancelCells(handle: number, uri: UriComponents, handles: number[]): Promise<void> {
		...
		if (obj.controller.interruptHandler) {
			await obj.controller.interruptHandler.call(obj.controller, document); // {{SQL CARBON EDIT}}
		}
	...
	$acceptRendererMessage(handle: number, editorId: string, message: any): void {
		...
		obj.onDidReceiveMessage.fire(Object.freeze({ editor: editor, message })); // {{SQL CARBON EDIT}}
	}
}
